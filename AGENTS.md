...
- INPUT, RULES, STATE, OUTPUT, the continuous shaft, and the stone are persistent transparent layers. Every expanded component has one compact counterpart and one reversible transform; no endpoint image swaps or staged visibility reveals are used.
- Each moving component owns a fixed viewport mask so the compact shell occludes parts cleanly while the enclosure opens. Leaving or blurring reverses from the current spring position and velocity.
...
- In the expanded state, the RULES gear train runs as a restrained counter-rotating mechanical cycle. It fades and pauses during reassembly and stays still for reduced-motion users.
- In the expanded state, INPUT's flywheel, RULES' counter-rotating gears, STATE's front drum, and OUTPUT's lattice sphere each own a distinct restrained cycle. RULES stays clearly legible at normal page scale; the cycles ramp up only after the parts separate and pause when reassembling, offscreen, page-hidden, or reduced-motion.
- Internal motion uses fine-grained layer ownership: shafts, central hubs, outer silhouettes, rims, and fixed photographic lighting remain static while approved surface details rotate around the measured shaft center. Every transformed sprite uses a 4× RGBA design master and a tight 3× lossless runtime asset, alpha-safe resampling, and a fixed viewport mask.
- INPUT and STATE follow seamless 360-degree rigid-body rotation about their measured shaft axes. RULES preserves tooth-mesh phase and derives counter-rotation speed from pitch-radius or tooth-count ratios; visibly coaxial connected bodies share angular velocity. OUTPUT follows rigid-body sphere rotation about its center with quaternion-equivalent orientation, stable angular velocity, correct depth ordering, spatial occlusion, and world-fixed photographic lighting.
- CSS/WAAPI is the default for projected planar mechanisms. Three.js is reserved for components that require true 3D geometry, dynamic lighting, camera parallax, or spatial occlusion, with OUTPUT as the first explicit decision gate.
